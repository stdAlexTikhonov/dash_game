...
import {
  ...
  ELECTRON,
  ORANGE_DISK,
  ORANGE_DISK_QUANTITY,
  BOMB_QUANTITY,
  RED_DISK,
  PC,
  YELLOW_DISK_QUANTITY,
  YELLOW_DISK,
  BUGS_QUANTITY,
  BUG,
  PORTAL,
...
export class World {
        ...
        try {
          ...
          switch (res.method) {
            case "SET_PLAYERS":
              ...
              if (Object.keys(res.players).length === players_quantity - 1)
                this.start = true;
              break;
            case "CD":
  ...
  generate() {
    ...
    MIDDLE[0] = { char: WALL, img: wall_img };
    MIDDLE[this.width - 1] = { char: WALL, img: wall_img };
  ...
  print(value) {
    ...
    const viewport_start_x =
      this.player.x -
      Math.floor(
        settings.orientation ? VIEWPORT_WIDTH / 2 : VIEWPORT_HEIGHT / 2
      );
    const viewport_start_y =
      this.player.y -
      Math.floor(
        settings.orientation ? VIEWPORT_HEIGHT / 2 : VIEWPORT_WIDTH / 2
      );
    const viewport_end_x =
      viewport_start_x +
      (settings.orientation ? VIEWPORT_WIDTH : VIEWPORT_HEIGHT);
    const viewport_end_y =
      viewport_start_y +
      (settings.orientation ? VIEWPORT_HEIGHT : VIEWPORT_WIDTH);

    this.world.forEach((row, i) => {
      const draw_view_y_flag = i >= viewport_start_y - 1 && i <= viewport_end_y;
      row.forEach((el, j) => {
        const draw_view_x_flag =
          j >= viewport_start_x - 1 && j <= viewport_end_x;
        if (draw_view_y_flag && draw_view_x_flag) {
          ...
          switch (el.char) {
            case SCISSORS:
              ...
              this.ctx_vp.drawImage(
                el.img,
                BLOCK_WIDTH * el.state,
                DIRS.indexOf(el.dir) * BLOCK_WIDTH,
                BLOCK_WIDTH,
                BLOCK_WIDTH,
                pos_x,
                pos_y,
    ...
    return (
      this.world.map((row) => row.join(EMPTY)).join("\n") +
      "\nscores: " +
  ...
  check_predators() {
    ...
    if (died) {
      ...
      this.GROUND = this.GROUND.filter(
        (G) => !arr.some((el) => el.x === G.x && el.y === G.y)
      );
      this.ROCKS = this.ROCKS.filter(
        (G) => !arr.some((el) => el.x === G.x && el.y === G.y)
      );
      this.DISKS = this.DISKS.filter(
        (G) => !arr.some((el) => el.x === G.x && el.y === G.y)
      );
      ...
      this.BREAKS = this.BREAKS.filter(
        (G) => !arr.some((el) => el.x === G.x && el.y === G.y)
      );
      ...
      Player.off = arr.some(
        (el) => el.x === this.player.x && el.y === this.player.y
      );
    }
  ...
  check_bombs() {
    ...
    if (died) {
      ...
      this.DISKS = this.DISKS.map((G) =>
        arr.some((el) => el.x === G.x && el.y === G.y)
          ? { ...G, still_alive: false }
          : G
      );
      this.STARS = this.STARS.filter(
      ...
      this.ELECTRONS = this.ELECTRONS.map((G) =>
        arr.some((el) => el.x === G.x && el.y === G.y)
          ? { ...G, still_alive: false }
          : G
      );
  ...
  check_bugs() {
    ...
    const died = this.BUGS.find(
      (bug) => x === bug.x && y === bug.y && bug.killer
    );
    if (died) {
      this.EXPLOSIONS.push(new Explosion(died.y, died.x));
      this.EXPLOSIONS.push(new Explosion(died.y + 1, died.x));
    ...
    if (just_to_filter)
      this.BUGS = this.BUGS.filter(
        (el) => el.x !== just_to_filter.x && el.y !== just_to_filter.y
      );
  }
